...
5. `upload_certs.yml` — uploads TLS cert/key and CA cert; also uploads dedicated Etcd TLS certs when `gateway_server_store_etcd_use_tls: true` and the matching `gateway_server_store_etcd_*_cert_src`/`_key_src` vars are set (falls back to reusing the main Gateway PKI cert/key/CA for Etcd otherwise, unchanged from prior behavior)
