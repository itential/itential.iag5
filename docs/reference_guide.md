...
| `gateway_server_store_etcd_ca_cert_src` | String | The path on the control node to a dedicated Etcd CA certificate. When set, the role uploads it to the target host; when unset, no Etcd-specific CA cert is uploaded and `gateway_server_store_etcd_ca_certificate_filename` falls back to the main Gateway CA cert. | N/A |
| `gateway_server_store_etcd_cert_src` | String | The path on the control node to a dedicated Etcd client certificate. Same fallback behavior as `gateway_server_store_etcd_ca_cert_src`. | N/A |
| `gateway_server_store_etcd_key_src` | String | The path on the control node to a dedicated Etcd client private key. Same fallback behavior as `gateway_server_store_etcd_ca_cert_src`. | N/A |
| `gateway_server_store_etcd_ca_certificate_filename` | String | The path to the CA certificate used to authenticate to the Etcd store backend. Resolves to a dedicated path (`{{ gateway_server_pki_dir }}/etcd-ca{{ gateway_pki_cert_suffix }}`) when `gateway_server_store_etcd_ca_cert_src` is set, otherwise falls back to the main Gateway CA cert (today's default, unchanged). | "{{ gateway_server_pki_ca_cert_dest }}" (or the dedicated path, if opted in) |
| `gateway_server_store_etcd_certificate_filename` | String | The path to the client certificate used to authenticate to the Etcd store backend. Same fallback behavior, keyed on `gateway_server_store_etcd_cert_src`. | "{{ gateway_server_pki_cert_dest }}" (or the dedicated path, if opted in) |
| `gateway_server_store_etcd_private_key_filename` | String | The path to the client private key used to authenticate to the Etcd store backend. Same fallback behavior, keyed on `gateway_server_store_etcd_key_src`. | "{{ gateway_server_pki_key_dest }}" (or the dedicated path, if opted in) |
